...
const title = ref('Usuários Registrados')
const alerts = ref([])
let search = ref({ name: '', extension: '', department: '' })
let selected_user = ref(null)
let users = ref({})
let dropdown_options = ref({ index : '', show : false})
let show_modal_delete = ref(false)

const getUsers = debounce(async function(url=1, success=false){
    loading.value = true
    let params = ""

    if(search.value.name !== "")
        params += params + '&name=' + search.value.name
    if(search.value.extension !== "")
        params += params + '&extension=' + search.value.extension
    if(search.value.department !== "")
        params += params + '&department=' + search.value.department

    await axios.get('api/users?page='+url+params)
    .then((response) => {
...
const confirmDelete = async () => {
    loading.value = true
    clearAlerts()
    router.delete(route('user.destroy', selected_user.value.id ), {
        onSuccess: async () => {
            getUsers(),
            getFlashAlerts(),
                                        ...
                                        <div class="inline-block text-left">
                                            ...
                                            <transition name="slide-fade">
                                                ...
                                                >
                                                    ...
                                                    <div>
                                                        <button
                                                            @click="show_modal_delete = true, selected_user = user, show_options"
                                                            type="button" 
                                                            role="menuitem" 
                                                            tabindex="-1" 
                                                            id="menu-item-0" 
                                            ...
                                            <div class="flex justify-end mb-2 absolute top-1 right-1.5">
                                                ...
                                                <transition name="slide-fade">
                                                    ...
                                                    >
                                                        ...
                                                        <div role="none">
                                                            <button
                                                                @click="show_modal_delete = true, selected_user = user, dropdown_options.show = false, dropdown_options.index = ''"  
                                                                :color="'flex items-center justify-center px-3 py-2 text-sm font-bold text-gray-700 border-b border-gray-200 rounded w-36 hover:text-gray-500'"
                                                                :width="20" 
                                                                :height="10"
                                                                type="button" 
                ...
                >
                    ...
                    <template #content>
                        ...
                        >
                            ...
                            />
                            <p class="mt-4">
                                Deletando usuário, aguarde um momento...
                            </p>
                        </div>
                        <div
                            v-else-if="!loading"
